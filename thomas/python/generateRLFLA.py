...
def antenna_like_RLFLA(x:float, y:float, z:float, polarisation:str, azimuth:float, inclination:float,
                       timeRemove:float, timeDelay:float = 0, isTx:bool = True, ID:str = 'RLFLA'):
    '''
    ...
    radiusFeedVoid = radiusWire    
    lengthFeedVoid = 0.02               # originally 0.01m, cut-off for performance
    ...
    edge(xs=x-lengthWire, ys=y+radiusAn, zs=z,           # add deltaXRes to y2 due to discetization error with 0.01m
             xf=x+lengthWire, yf=y+radiusAn, zf=z,
             material='pec')

    # Place resistors
    dx             = (lengthWire - resistorLength) 
    for iRes in range(nResistor):   # from left to center     
        cylinder(x1=x-dx, y1=y+radiusAn, z1=z,               # resistor paralell  to wire
            x2=x-lengthRes-dx ,y2 =y+radiusAn,z2=z,
    ...
    edge(xs=x-0.5*lengthFeedVoid, ys=y+radiusAn, zs=z,           # add deltaXRes to y2 due to discetization error with 0.01m
         xf=x+0.5*lengthFeedVoid, yf=y+radiusAn, zf=z,
            material='air' + ID)


    cylinder(x1=x-0.5*lengthFeedVoid, y1=y+radiusAn, z1=z,
             x2=x+0.5*lengthFeedVoid, y2=y+radiusAn, z2=z,
             radius=radiusFeedVoid/2, material='air' + ID)
    ...
    if isTx:    # Source
        ...
        tx = voltage_source(polarisation=polarisation, f1=x, f2=y + radiusAn, f3= z, resistance = resSrc,
