function[dexp_mute,tap_exp2]=mute_second_event2(dexp_obs,ntrace,nsamp,t,trainv)
...
if scale_mode==1;
    ...
    scale_dexp_obs = dexp_obs;
end

if exist('muting2_picks.mat');
    load('muting2_picks.mat');
else
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    fprintf('Please select 2 points\n')
    [ta xa ]=ginput(2); % Anja: getauscht

    ta0=xa(1);
    xa0=round(trace(1));
    tan=xa(end);
    xan=round(trace(2));
    save('muting2_picks','ta0','xa0','tan','xan');
end    
...
if mode==0 || mode==1;
    for ntr=1:ntrace
        fall=Ta(ntr);
        high=Ta(ntr)+taplen;
        tap=transpose([ones(1,fall) 0.5*(1+cos(pi/(double(high)-double(fall)+1)*(1:double(high)-double(fall)+1))) zeros(1,nsamp-(double(high)+1))]);
        dexp_mute(:,ntr)=dexp_obs(:,ntr).*tap(1:nsamp,1);
        tap_exp2(:,ntr)=tap(1:nsamp,1);
...
elseif mode==5
    for ntr=1:ntrace
        ...
        tap=ones(nsamp,1);
        tap_ones=[zeros(1,low) 0.5*(1-cos(pi/(rise-(low))*(1:rise-(low))))];
...
%plot%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    imagesc(scale_dexp_mute);hold on
        title('mute second event');
        xlabel('Traces');
        ylabel('Samples');
        set(gca,'xaxislocation','top'); 
...
% save('..\fwi_data\tap_exp2','tap_exp2');
save(fullfile(script_dir,'tap_exp2'),'tap_exp2');